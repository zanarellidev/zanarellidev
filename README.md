<img src="./assets/octopus-network.png" alt="Polvo rosa em pixel art operando um terminal no meio de uma rede imensa de computadores conectados" width="100%" />

# Raphael Zanarelli

Staff Software Engineer especializado em Cloud, Segurança e IA.

Engenheiro de software com mais de 10 anos criando e operando aplicações empresariais críticas. Combino arquitetura cloud-native, segurança, observabilidade e liderança técnica.

[Currículo](https://zanarellidev.github.io) · [LinkedIn](https://www.linkedin.com/in/raphaelzanarelli/) · [GitHub](https://github.com/zanarellidev)

## Contribuições recentes em open source

Pull requests em projetos de infraestrutura e plataformas distribuídas:

- [gRPC-Go](https://github.com/grpc/grpc-go/pull/9275) — garantia de invocação única para callback `OnFinish`.
- [cert-manager](https://github.com/cert-manager/cert-manager/pull/9081) — correção de `nil pointer` em validação de identidade.
- [Rook](https://github.com/rook/rook/pull/18066) — correção no wipe de dispositivos criptografados.
- [OpenTelemetry Collector](https://github.com/open-telemetry/opentelemetry-collector/pull/15679) — validação de IDs em feature gates.
- [etcd](https://github.com/etcd-io/etcd/pull/22211) — preservação de estado de líder em falhas de transação.
- [Kubernetes](https://github.com/kubernetes/kubernetes/pull/140991) — cancelamento de probes em execução ao parar o worker.
- [Cilium](https://github.com/cilium/cilium/pull/47568) — liberação de node ID órfão após remapeamento parcial.
- [Istio](https://github.com/istio/istio/pull/61024) — persistência do backend iptables no CNI.

## Quer me contatar por algum desses motivos?

- [Tenho uma ideia para compartilhar](https://github.com/zanarellidev/zanarellidev/issues/new?template=idea.yml)
- [Quero discutir uma dúvida técnica](https://github.com/zanarellidev/zanarellidev/issues/new?template=technical-sparring.yml)
- [Quero uma perspectiva de carreira](https://github.com/zanarellidev/zanarellidev/issues/new?template=career.yml)
- [Quero propor uma colaboração](https://github.com/zanarellidev/zanarellidev/issues/new?template=collaboration.yml)
- [Quero conversar sobre contratação ou parceria](https://www.linkedin.com/in/raphaelzanarelli/)
